...
I0=I0.*exp(1i*PHASE);
%% Noise
snr=0.01; % White gaussian noise
wellDepth=40000;
G_AD=wellDepth/M % e-/px (Gain)
F_Ill=1 % 400e-/px
Ref_Ill=40 % 32000e-/px
%% Showing CCD FOV
I_FOV=zeros(M);
...
IH=(EOf+Ref).*conj(EOf+Ref)/G_AD+awgn(abs(Ref)/G_AD,snr); % |F+R|^2 + noise
scale=.5; % pad3/pad2
...
% QP_factor=1;
QP_factor=1;%=exp(1i*2*pi*z/w)*exp(1i*pi*((C3-M*pad_w/2-1).^2+(R3-M*pad_w/2-1).^2)/w/z)/1i/w/z;
